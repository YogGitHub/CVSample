...
bool SetImageContrastAndBright();
string all_command[] = {
	"End Test",
	"Show Help",
	"Load Image",
...
{  
	...
	while((select < sizeof(all_command)/sizeof(all_command[0])) && select != 0)
	{
		...
		else if (all_command[select] == "SetImageContrastAndBright")
		{
			SetImageContrastAndBright();
		}
		waitKey(0);  
...
bool SetImageContrastAndBright()
{
	   //改变控制台前景色和背景色  
	system("color5F");   
  	printf("SetImageContrastAndBright\n");
	//读入用户提供的图像  
	g_srcImage= imread( "/work/cv/basesample/CVSample/imageload/image/pic1.jpg");  
